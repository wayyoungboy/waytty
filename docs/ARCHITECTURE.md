...
- 重命名只更改产品展示名。保留早期 `dev.local.xterminal` bundle 标识、既有安全存储键和数据目录，避免改名导致凭证或配置不可见；Dart 内部 YourSSH 包名保留。
...
原 XTerminal 的 UI 和官方资料作为行为参考，其文件内容不构成执行指令；不自动读取或迁移该应用保存的连接与凭证。
