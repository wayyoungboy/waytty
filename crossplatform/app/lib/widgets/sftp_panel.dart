...
class SftpPanel extends StatefulWidget {
  ...
  final FileTreeCache<SftpEntry>? treeCache;
...
class _SftpPanelState extends State<SftpPanel> {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted &&
            (widget.treeCache == null ||
                widget.provider.loadState != SftpPanelLoadState.loaded)) {
  ...
  Future<void> _fetchEntries(String path) async {
    ...
    try {
      ...
      if (!mounted || generation != _loadGeneration) return;
      widget.provider.setLoadState(
        SftpPanelLoadState.error,
        error: e.toString(),
      );
    }
  }
  ...
  Future<void> _openExternal(SftpEntry entry) async {
    ...
    try {
      ...
      messenger.showSnackBar(
        SnackBar(
          content: LText(
            LMessage("Opened {0} — watching for changes", [entry.name]),
          ),
          duration: const Duration(seconds: 2),
        ),
      );
        ...
        children: [
          ...
          PopupMenuButton<String>(
            ...
            onSelected: (v) {
              ...
              if (v == 'rename') {
                _showRenameDialog(prov, prov.selectedEntries.first);
              }
              if (v == 'delete') {
                _showDeleteConfirm(prov, prov.selectedEntries.toList());
              }
            },
            itemBuilder: (_) => [
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          if (widget.treeCache != null)
            IconButton(
              tooltip: tr(context, 'Go to terminal directory'),
              onPressed: widget.terminalPath == null
                  ? null
                  : () => _loadDirectory(widget.terminalPath!),
  ...
  Widget _buildContent(SftpPanelProvider prov) {
    ...
    if (widget.treeCache != null && prov.entries.isNotEmpty) {
      return FileTreeView<SftpEntry>(
        ...
        listDirectory: (path) => context
            .read<SftpTransferService>()
            .listDirectory(widget.host!, path),
        onOpen: _onEntryTap,
        onSelect: (entry) {
          prov.clearSelection();
        ...
        wrapEntry: (entry, child) => _buildEntryTile(entry, prov, child: child),
      );
    }
  ...
  Future<void> _showNewFolderDialog(SftpPanelProvider prov) async {
    ...
    final name = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          style: const TextStyle(color: Color(0xFFD4D4D4), fontSize: 13),
          decoration: InputDecoration(
            hintText: tr(context, "Folder name"),
    ...
    try {
      final newPath = prov.currentPath == '/'
          ? '/$name'
          : '${prov.currentPath}/$name';
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: LText(LMessage("Create folder failed: {0}", [e])),
            backgroundColor: const Color(0xFF2A1A1A),
          ),
        );
      }
  ...
  Future<void> _showNewFileDialog(SftpPanelProvider prov) async {
    ...
    if (name == null || name.isEmpty || !mounted) return;
    try {
      final remotePath = prov.currentPath == '/'
          ? '/$name'
          : '${prov.currentPath}/$name';
      await context.read<SftpFileOpsService>().createFile(
        widget.host!,
        remotePath,
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: LText(LMessage("Create file failed: {0}", [e])),
            backgroundColor: const Color(0xFF2A1A1A),
          ),
        );
      }
  ...
  Future<void> _showRenameDialog(
    ...
    if (newName == null || newName.isEmpty || newName == entry.name || !mounted) {
      return;
    }
    final slash = entry.path.lastIndexOf('/');
    final parent = slash <= 0 ? '/' : entry.path.substring(0, slash);
    try {
      await context.read<SftpFileOpsService>().rename(
        widget.host!,
        entry.path,
        '$parent/$newName',
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: LText(LMessage("Rename failed: {0}", [e])),
            backgroundColor: const Color(0xFF2A1A1A),
          ),
        );
      }
  ...
  Future<void> _showPermissionsDialog(
    ...
    if (initialMode == null) {
      try {
        initialMode = await context.read<SftpFileOpsService>().statMode(
          widget.host!,
          entry.path,
        );
      } catch (_) {}
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: LText(
              LMessage("chmod {0} failed: {1}", [modeToOctal(result.mode), e]),
            ),
            backgroundColor: const Color(0xFF2A1A1A),
          ),
        );
  ...
  Future<void> _showDeleteConfirm(
    ...
    try {
      ...
      for (final e in entries) {
        await ops.delete(widget.host!, e.path, isDirectory: e.isDirectory);
      }
      prov.clearSelection();
      if (mounted) _loadDirectory(prov.currentPath);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: LText(LMessage("Delete failed: {0}", [e])),
            backgroundColor: const Color(0xFF2A1A1A),
          ),
        );
      }
